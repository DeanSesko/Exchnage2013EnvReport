...
$Output+="<table border=""0"" cellpadding=""3"" width=""100%"" style=""font-size:8pt;font-family:Arial,sans-serif"">
<col width=""30%""><col width=""10%""><col width=""30%""><col width=""30%""><col width=""30%"">
    	 <tr align=""left"" bgcolor=""#008080""><th>CAS Server Urls</th><th>Path</th><th>Internal</th><th>External</th></table>"
$Output+="<table border=""0"" cellpadding=""3"" width=""100%"" style=""font-size:8pt;font-family:Arial,sans-serif"">
<col width=""30%""><col width=""10%""><col width=""30%""><col width=""30%""><col width=""30%""><tr align=""left"">"




foreach($Exchserver in Get-ExchangeServer )
{
	[array]$Roles = $ExchServer.ServerRole.ToString().Replace(" ","").Split(",")
	
		if ($Roles -contains "ClientAccess")
		{
			$OWACASServer = Get-OwaVirtualDirectory -Server $ExchServer.name |Select  internalUrl,Externalurl,name,Server
			[String]$OWAINT = $OWACASServer.internalurl.AbsoluteURI
			[String]$OWAEXT = $OWACASServer.Externalurl.AbsoluteURI
			[String]$OWAServerName = $OWACASServer.Server.Name
			[String]$OWAPath = "OWA"
			$Output+="<td>$OWAServerName</td><td>$OWAPath</td><td>$OWAINT</td><td>$OWAEXT</td></tr>"
			$OABCASServer = Get-OaBVirtualDirectory -Server $ExchServer.name |Select  internalUrl,Externalurl,name,Server
			[String]$OABINT = $OABCASServer.internalurl.AbsoluteURI
			[String]$OABEXT = $OABCASServer.Externalurl.AbsoluteURI
			[String]$OABServerName = $OABCASServer.Server.Name
			[String]$OABPath = "OAB"
			$Output+="<tr style=background-color:#dddddd ><td>$OABServerName</td><td>$OABPath</td><td>$OABINT</td><td>$OABEXT</td></tr>"
			$EWSCASServer = Get-WebServicesVirtualDirectory -Server $ExchServer.name |Select  internalUrl,Externalurl,name,Server
			[String]$EWSINT = $EWSCASServer.internalurl.AbsoluteURI
			[String]$EWSEXT = $EWSCASServer.Externalurl.AbsoluteURI
			[String]$EWSServerName = $EWSCASServer.Server.Name
			[String]$EWSPath = "EWS"
			$Output+="<td>$EWSServerName</td><td>$EWSPath</td><td>$EWSINT</td><td>$EWSEXT</td></tr>"
			$EASCASServer = Get-ActiveSyncVirtualDirectory -Server $ExchServer.name |Select  internalUrl,Externalurl,name,Server
			[String]$EASINT = $EASCASServer.internalurl.AbsoluteURI
			[String]$EASEXT = $EASCASServer.Externalurl.AbsoluteURI
			[String]$EASServerName = $EASCASServer.Server.Name
			[String]$EASPath = "Exchange ActiveSync"
			$Output+="<tr style=background-color:#dddddd ><td>$EASServerName</td><td>$EASPath</td><td>$EASINT</td><td>$EASEXT</td></tr>"	}
}
...
$Output+="<table border=""0"" cellpadding=""3"" width=""100%"" style=""font-size:8pt;font-family:Arial,sans-serif"">
<col width=""30%""><col width=""10%""><col width=""30%""><col width=""30%""><col width=""30%"">
<tr align=""left"" bgcolor=""#E69900""><th>AutoDiscover Servers </th><th>URL</th></table>"
$Output+="<table border=""0"" cellpadding=""3"" width=""100%"" style=""font-size:8pt;font-family:Arial,sans-serif"">
<col width=""50%""><col width=""50%""><tr align=""left"">"
...
$Output+="<tr"
		...
		} else
		{
			...
			[String]$AutodURL = $casServer.autoDiscoverServiceInternalUri
			[String]$AutoDName = $casServer.Name
			

$Output+="><td>$AutoDName</td><td>$AutodURL</td>"
}
